...
static void dfs(int **image, int x, int y, int color, int newColor) {
    if (image[x][y] == color) {
        image[x][y] = newColor;
        for (int i = 0; i < 4; i++) {
...
static int **
floodFill(int **image, int imageSize, const int *imageColSize, int sr, int sc, int newColor, int *returnSize,
